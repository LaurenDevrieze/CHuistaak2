...
im_fine = integrators.implicit_midpoint_integrator.ImplicitMidpointIntegrator(ode1,newt2)
tfine,yfine = im_fine.integrate(y0,t0,t[-1]+hfine/2.,hfine)

ind = t/hfine
indices = ind.astype(int)

plt.figure()
plt.plot(t,y_ny[:,0]-yfine[indices,0])
plt.figure()
plt.plot(t,y_im[:,0]-yfine[indices,0])
