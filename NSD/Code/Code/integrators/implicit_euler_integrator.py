...
class ImplicitEulerIntegrator(iosi.ImplicitOneStepIntegrator):
    """ 
    ...
    def __init__(self,ode,solver):
        ...
        super(ImplicitEulerIntegrator,self).__init__(ode,solver)
        # self.nu needs to contain the number of stages
        self.nu = 2
        self.c = scipy.array([0.,1.])
        self.b = scipy.array([0.,1.])
        self.constant = -1./12.
    ...
    def g(self,w):
        ...
        #result[neq:]= self.ode.f(tc[1],w[:neq])
	result[neq:]= 0*self.ode.f(tc[0],w[:neq])+\
    ...
    def g_jac(self,w):
        ...
        result[neq:,:neq] = 0*self.ode.Jacobian(tc[1],w[:neq])
        result[neq:,neq:] = 1.*self.ode.Jacobian(tc[1],w[neq:])
        return result
