...
class CHODE(ode_system.ODESystem):
    
    def __init__(self,lambd):
        """ 
        creates an object of the type: Curtiss-Hirschfelder
        of the form y'=f(t,y)=lambd*(y- cos t)
        Input:
    ...
    def f(self,t,y):
        ...
        """
        return self.lambd*(y - scipy.cos(t))
    ...
    def y_exact(self,time,t0,y0):
        ...
        nt = scipy.size(time)
        yt = scipy.zeros((nt,1))
        yt[:,0]= 2500./2501.*scipy.cos(time) + 50./2501.*scipy.sin(time) +\
        1./2501.*scipy.exp(self.lambd*time)
        return yt
